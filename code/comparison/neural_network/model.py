...
from comparison.neural_network.perceptron import Perceptron, UpdatingPerceptron, InputPerceptron
from comparison.neural_network.utils import Vector, Matrix, cols
...
class MLPClassifier:
    ...
    def __init__(self, *, n_layers: int, layer_size: int, learning_rate: float = 0.3, n_generations: int = 100):
        ...
        self.outcome: Optional[Vector] = None
        """
        A saída esperada para essa rede neural.
    ...
    def fit(self, X: Matrix, y: Vector) -> 'MLPClassifier':
        ...
        for _ in range(self.n_generations):
            for sample, outcome in zip(X, y):
                self._update_weights(sample, outcome)
    ...
    def predict(self, X: Union[Vector, Matrix]) -> Union[float, Vector]:
        ...
        for feature in X:
            ...
            outcome = min(y, key=lambda x: abs(x - value))
            if is_vector:
                return outcome
